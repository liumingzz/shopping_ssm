...
public interface UsersDao {
    ...
    @Select("<script>"+ "SELECT count(*) " + "from users" + "<where>" +
            "<if test='username!=null'>" +
            "and username like CONCAT('%',#{username},'%')" +
            "</if>" +
            "</where>" +
            "</script>")
    public long queryCount(@Param("username")String username);
    ...
    @Select("<script>"+ "SELECT * " + "from users" + "<where>" +
            ...
            "</script>")
    public List<Users> queryList(@Param("begin") int begin, @Param("size") int size,@Param("username")String username);
