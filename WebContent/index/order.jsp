                        ...
                        <tr class="order-info">
                            <td colspan="8">
                                <span style="margin-left: 20px;">订单编号&nbsp;:&nbsp;</span><i
                                    class="order-num">${order.id}</i>
                                <span style="margin-left: 40px;">下单时间&nbsp;:&nbsp;</span><i
                                    class="order-num"><fmt:formatDate value="${order.systime}"
                        ...
                        <tr class="order" style="margin-bottom: 10px;">
                            ...
                            <c:forEach items="${order.itemList}" var="item" end="0">
                                <td class="li1" style="text-align: left;" colspan="4">
                                    ...
                                    <div class="m-good-descript">
                                        <a href="detail?id=${item.goodId}"><p
                                                class="goods-descript">${item.good.name}</p></a>
                                    </div>
                            ...
                            <td class="li7" rowspan="${fn:length(itemList)}">
                                <div>
                                    ...
                                    <c:if test="${order.status==3}"><span>待收货</span></c:if>
                                    <c:if test="${order.status==4}"><span>待评论</span></c:if>
                                    <c:if test="${order.status==5}"><span>已完成</span></c:if>
                                </div>
                            </td>
                                ...
                                <div>
                                    <p>
                                        <c:if test="${order.status==1}">
                                            <a href="orderPay?id=${order.id}"><span
                                                    class="sta1 pay noSelect">确认付款</span></a>
                                        </c:if>

                                        <c:if test="${order.status==2}">
                                            <span style="color: forestgreen">等待发货...</span>
                                        </c:if>

                                        <c:if test="${order.status==3}">
                                            <a id="a_sure" href="../index/userconfirm?id=${order.id}&status=${order.status}">确认收货</a>
                                        </c:if>

                                        <c:if test="${order.status==4}">
                                            <a id="a_comment" href="../index/toComment?id=${order.id}">评价商品</a>
                                        </c:if>

                                        <c:if test="${order.status==5}">
                                            <a id="a_visitComment" href="../index/toComment?id=${order.id}">查看评论</a>
                                        </c:if>
                        ...
                        <c:forEach items="${order.itemList}" var="item" begin="1">
                            <tr class="order" style="margin-bottom: 10px;">
                                <td class="li1" style="text-align: left;" colspan="4">
                                    <a href="detail?id=${item.goodId}"><img src="${item.good.cover}"></a>
                                    <div class="m-good-descript">
                                        <a href="detail?id=${item.goodId}">
                                            <p class="goods-descript">${item.good.name}</p></a>
                                    </div>
                                    <div class="li3"><span>￥${item.price}</span></div>
                                    <div class="li4"><span>${item.amount}</span></div>
                                    <div class="li5"></div>
